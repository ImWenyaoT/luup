...
需要 Node.js：22 分支至少 22.13.0，23 分支至少 23.4.0，24 及以上主版本可用；包管理器使用 pnpm 10
（单体工作区由 Turborepo 编排）。
...
正式 live 批跑要求 Node.js 22 至少 22.13.0、Node.js 23 至少 23.4.0；Node.js 24 及以上可用。
启动前会校验版本，`--dry-run` 只做规划，不受版本门限制。
...
`--preflight` 复用正式付费启动门，额外检查上述 Node.js 能力范围、题库、确认参数、Qwen 凭据、clean release commit
