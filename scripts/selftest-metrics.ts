...
eq("q61 按时间序：20260810-052412 在后", groups.get(61)![6].id, "20260810-052412");

const p2 = passSquared(all);
// q61 七个 run → 6 个相邻对；只有 (062829,134046)=✓✓ 通过
eq("M5 相邻对总数 = 6（q61 有 7 个 run）", p2.total, 6);
eq("M5 通过的相邻对 = 1", p2.passed, 1);
eq("M5 Pass^2 = 1/6", Number(p2.rate!.toFixed(4)), 0.1667);
...
eq("累计保存 212 次", reuse.totalSaves, 212);
eq("去重后 132 篇", reuse.distinct, 132);
...
// (212-132)/212 = 0.37735…
eq("复用率 = 80/212", Number(reuse.reuseRate!.toFixed(4)), 0.3774);
 ...
 * 就破了，而且不会有任何报错。所以在这里逐字扫 lib/agents/ 全树（agent 可见面）。
 */
...
walkAgent(join(REPO_ROOT, "lib", "agents"));
check(`扫到 lib/agents/ 下 ${agentFiles.length} 个文件`, agentFiles.length > 0);
...
/* 与真实数据接：q61 七个 run —— 四个可交付版本都跑过 M9（062829 / 134046 /
   20260810-032527 报了 veto，20260810-052412 无 veto），另有三个 FAILED run
   （013424 / 042825 / 045543，不可交付）。veto 降为 advisory 之后不出局；
   唯一的出局理由只能是 deliverable=false。 */
...
eq("q61 四个可交付版本都跑过 M9", realCands.filter((c) => c.score !== null).length, 4);
eq("q61 报 veto 的版本 = 3（052412 无 veto）", realCands.filter((c) => c.veto).length, 3);
...
eq("q61 的真实校准未授权 M9，胜者由 refs 层分出（052412 有 10 篇）", realChoice.winner!.runId, "20260810-052412");
eq("q61 择优理由披露 M9 未达阈值", realChoice.reason, "M9 未达校准阈值，refs 更多");
check("胜者没有 veto（052412 是首个无 veto 版本），veto 只是诊断", !realChoice.winner!.veto);
eq("三版的 veto 都进 advisories", realChoice.advisories.length, 3);
eq("出局者 = 三个 FAILED run（因不可交付，绝不因 veto）", realChoice.eliminated.length, 3);
check("出局名单不含任何可交付版本", realChoice.eliminated.every((e) => ["20260810-013424", "20260810-042825", "20260810-045543"].includes(e.runId)));

// 只看 Tier1（不接 M9）时胜者相同，但分出胜负的层次不同 —— 两条路都要留住
const tier1Only = selectVersion(realCands.map((c) => ({ ...c, score: null, veto: false })));
eq("只看 Tier1 时 q61 同样由 refs 层分出", tier1Only.reason, "M9 未达校准阈值，refs 更多");
eq("只看 Tier1 时胜者同为 052412", tier1Only.winner!.runId, "20260810-052412");
