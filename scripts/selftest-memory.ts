...
import {
  ...
  memoryEnabled,
  memoryIndexPath,
  parseLibraryIndexRows,
  questionPath,
  searchMemory,
  writeNote,
} from "#lib/campaignMemory.ts";
import { RUN_DIR_ENV, listPapers, readIndex, savePaper } from "#lib/paperStore.ts";
...
check(
  ...
  existsSync(join(libraryPapersDir(memoryDir), "2401.11111.md")),
);
...
check("全局卡正文保留了 Abstract（run 卡正文原样搬运）", readFileSync(join(libraryPapersDir(memoryDir), "2401.11111.md"), "utf8").includes("## Abstract"));
...
const libIndex = readFileSync(libraryIndexPath(memoryDir), "utf8");
const rows = parseLibraryIndexRows(libIndex);
...
check("旧式 id 的 `/` → `__` 文件名映射一致", existsSync(join(libraryPapersDir(memoryDir), "astro-ph__0601001.md")));
check("index 顶部声明「代码重建，请勿手改」", libIndex.includes("自动重建，请勿手改"));
check("index 仍写明 B1 不放松", libIndex.includes("criteria B1"));
check("memory/index.md 已派生", existsSync(memoryIndexPath(memoryDir)) && readFileSync(memoryIndexPath(memoryDir), "utf8").includes("library/index.md"));
...
check("index 仍是 2 行（upsert 不重复建行）", parseLibraryIndexRows(readFileSync(libraryIndexPath(memoryDir), "utf8")).length === 2);
check(
  "index 行含反向索引 q54 q61",
  parseLibraryIndexRows(readFileSync(libraryIndexPath(memoryDir), "utf8")).some((r) => r.questions.includes("q54") && r.questions.includes("q61")),
);
...
const q54 = readFileSync(questionPath(54, memoryDir), "utf8");
...
const q54b = readFileSync(questionPath(54, memoryDir), "utf8");
...
appendLog({ action: "run", questionId: 54, verdict: "SUCCESS", detail: "runs/20260808-000000 refs=7" });
const logText = readFileSync(logPath(memoryDir), "utf8");
...
check("q61.md 含 verdict 与 run 指针", readFileSync(questionPath(61, memoryDir), "utf8").includes("verdict: FAILED") && readFileSync(questionPath(61, memoryDir), "utf8").includes(runDir));
...
try {
  ...
  const l = appendLog({ action: "run", questionId: 54, verdict: "SUCCESS" });
  noop.log = "skipped" in l && l.skipped === true;

  const n = appendQuestionNote({ questionId: 54, note: "should be dropped" });
  noop.note = "skipped" in n && n.skipped === true;
...
check("appendLog 静默 skipped", noop.log);
check("appendQuestionNote 静默 skipped", noop.note);
