/**
 * .eve/.workflow-data 磁盘保留策略（125 全量跑的前置修复）。
 *
...
export function indexRuns(runsDir: string): RunInfo[] {
  const out: RunInfo[] = [];
...
export function planPrune(opts: PruneOptions = {}): PruneResult {
  const stateDir = opts.stateDir ?? join(repoRoot, ".eve", ".workflow-data");
  const runsDir = opts.runsDir ?? join(repoRoot, "runs");
...
/** run-batch.ts 的入口：planPrune + 一行摘要，不打整张表。 */
export function pruneEveState(opts: PruneOptions = {}): PruneResult {
  return planPrune(opts);
}
...
function main(argv: string[]): number {
  ...
  const r = planPrune({ apply, graceMs: graceMin * 60 * 1000 });
...
// 直接执行才跑 CLI；被 run-batch.ts import 时只暴露函数
