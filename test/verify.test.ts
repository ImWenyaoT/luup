...
test("B1 accepts only references frozen in this run's evidence", () => {
  ...
  assert.deepEqual(checkFrozenMembership(targets, cards.size).map((check) => check.pass),
...
test("B2 passes when the frozen title still matches an independent arXiv lookup", () => {
  ...
  assert.deepEqual(checks.map((check) => check.id), ["B2.2301.12345"]);
...
test("B4 passes when authors and year still agree with arXiv", () => {
  ...
  assert.deepEqual(checks.map((check) => check.id), ["B4.2301.12345"]);
  assert.equal(checks[0]!.pass, true);
});

test("B4 reports a year mismatch, a bogus author and a wrong first author", () => {
  const targets = resolveTargets([card.url!], collectFrozenCards([{
    ...card,
    year: 1999,
...
const lookupOf = (...records: ResolvedRecord[]): ArxivLookup => async (ids) => {
...
test("layers web references as membership-only instead of pretending they were looked up", async () => {
  const verify = createReferenceVerifier({ lookup: lookupOf(remote) });
  const verification = await verify({
    plan: plan([card.url!, doiCard.url!]),
    research: research([card, doiCard]),
  });

  assert.equal(verification.arxivChecked, 1);
  assert.equal(verification.membershipOnly, 1);
  // DOI 引用只有 B1，没有 B2/B4 —— 报告里的分层就是从这里来的
...
test("never looks up a reference that is not in the frozen evidence", async () => {
  ...
  const verify = createReferenceVerifier({
    lookup: async (ids) => { asked = ids; return []; },
  });
  await verify({
...
test("marks an arXiv outage as infrastructure failure, not fabrication", async () => {
  const verify = createReferenceVerifier({
    lookup: async () => { throw new ArxivLookupError("arXiv lookup returned HTTP 503"); },
  });
...
test("chooses the failure code by what actually went wrong", () => {
  ...
  assert.equal(verificationFailureCode({ ...base, failed: ["B2.resolve"], infraError: true }),
    "infra_error");
  // 网络好坏改变不了 B1/B3 的结论
  assert.equal(
  ...
  assert.equal(verificationFailureCode({ ...base, failed: ["B2.2301.12345"], infraError: false }),
    "verifier_refs");
});
