...
| **handoff**          | Luup 控制面的结构化交接：角色之间只传冻结 Artifact，不共享隐藏上下文；目前不是 SDK 控制权转移                                        | `apps/server/src/harness.ts` 的 `toInput` |
...
| **HTTP adapter**     | Elysia / Node HTTP 独立暴露运行接口；Next.js 通过同源 rewrite 代理 `/api`、`/health`、`/readyz`，前端只消费 HTTP                     | `apps/server/src/server.ts`               |
