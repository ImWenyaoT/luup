...
现行仓库已经不满足 ADR-0007 的验收条件：根包声明 `engines.node >=24.20.0` 与
`packageManager=pnpm@11.24.0`，工作区由 `pnpm-workspace.yaml` 和 Turborepo 编排，CI 在 Node.js 24.20.0
...
Node.js 24.20.0 是 2026-08-30 的 Node 24 Latest LTS；以当前 LTS patch 为最低版本可以同时满足
生产支持线与 `node:sqlite` 无实验 flag 的要求，并删除 22/23 分支特判。
...
- API、Harness、批跑、评估与测试要求 Node.js >=24.20.0；正式 live batch 在读取模型凭据或
  打开数据库前拒绝更早版本和畸形版本字符串。
- pnpm 11.24.0 是唯一包管理器，`package.json#packageManager`、`pnpm-lock.yaml` 与
...
- CI 显式安装最低受支持版本 Node.js 24.20.0 和 pnpm，且不安装 Bun；
