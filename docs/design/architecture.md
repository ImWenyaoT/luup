...
apps/web (Vite/React)
        ↓ HTTP + SSE
src/server.ts ── src/harness.ts ── researcher
   （同进程）          │            ├─ hypothesis-generation
                      │            ├─ evidence-review
                      │            ├─ research-plan
                      │            └─ reviewer
                      ├─ src/agent/tools/  arXiv + Crossref 检索
                      ├─ src/verify/       B1–B4 确定性引用验收（零 LLM）
                      └─ src/store/        node:sqlite 单文件事实存储
```
...
外部 interface 只有「给定 run id，推进到终态」。`src/harness.ts` 拥有：
...
角色的合同与提示词在 `src/agent/roles/`（每个角色一对 `.ts` + `.md`），
结构化输出的形状在 `roles/structured-output.ts`。harness 是运行时角色，不是子目录。

### seam

可替换的东西全在 `src/seams/index.ts` 点名，只有类型与一个工厂——每个接缝目前只有
...
`src/server.ts` 是一个 `node:http` 服务器，同时做四件事：输入防护、两槽并发闸、
...
- 公开状态收窄为 `running → completed | review_rejected | failed`（`src/store/schema.ts`）。

`src/api/projection.ts` 是对外的字段 allowlist：审计与恢复字段不出网，未知字段默认不公开。

### Web adapter

`apps/web/` 只通过 HTTP 读 run 快照、事件与 Artifact，不直接推断存储状态。
wire type 手写在 `apps/web/src/types.ts`，**不从服务端模块导入类型**——这条边界是故意的，
它让投影的收窄在两侧都是显式的。生产形态下 `dist/` 由 `src/server.ts` 同端口托管，
只有一个进程。
...
4. **research-plan** 产出研究计划，引用必须能追溯到冻结 Artifact 的 citations
   （`src/agent/plan-quality.ts` 的两道门）。
...
- **SQLite 单文件**是运行事实源：`src/store/`，默认 `outputs/runtime/typescript-runs.db`，
...
单进程内两个并发槽（`src/server.ts` 的 `maxConcurrentRuns`）：够演示并发，
又不会让一批请求同时放大成无上限的付费调用。批跑走另一条路——`src/batch/runner.ts`
...
`src/eval/metrics.ts` 只读一个跑完的 SQLite 库，纯函数，不调模型或网络：
版本择优链 gate → refs → token → run id；Tier1 聚合与失败分类分组；
`firstVsLatest` 与 `memoryArms` 两种 McNemar 精确配对。

失败分类的权威定义是 `src/agent/failures.ts`；`INFRASTRUCTURE_CLASSES` 在
`metrics.ts` 里**写成字面量而非 import**——改 agent 不该改掉历史跑批的读数。

`src/eval/scoring.ts` 给单个 Run 打过程分（六分制），与 metrics 故意不互相 import：
评估口径必须比被评的代码更稳定。
