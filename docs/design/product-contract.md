...
官网 FAQ 明确要求参赛方自行部署可交互前端和测试 API；演示视频是可选附加项。Luup 因此把前端、测试 API 与可复核运行轨迹视为正式交付面，部署事实必须以真实公网验收为准，不能用本地可运行或 runbook 代替；演示视频仍不制作，也不扩张移动端或无障碍专项。
...
- 前端/API 是官方要求的交付面，但不得反向增加 Agent 复杂度；只做满足评审复核所需的部署，不做移动端、无障碍或通用实时通信专项；
