...
| Tier2 | M9 质量评分 | 四维（假设可证伪性/推导自洽/方案可落地/引用支撑度）×四级 rubric，LLM-judge 产分 + 断言归因；虚构类断言 = veto。分数落 run + 题页 | 版本择优、让 A 段的 judge 承诺成真 |
...
- **自进化闭环**（全自动）：run → M9 评分 → 题页 memory（**只回传事实不回传分数**：胜出假设、被拒原因、检索有效性）→ 重跑消费 → 版本择优纯函数（gate → M9 → refs 数 → token 成本，字典序）。
