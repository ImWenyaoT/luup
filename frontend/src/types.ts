...
export type Verdict = { file: string; node: string; round: number; verdict: "pass" | "reject"; checks: VerdictCheck[]; rework: string | null; rejectedRaw: string | null };
...
export type RunStatusView = { id: string; status: RunStatus; updatedAt: string; nodes: SpineNode[]; verdicts: Verdict[]; logTail: string[] };
...
export type Science125 = { source: string; retrievedAt: string; total: number; domains: Science125Domain[] };
export type ApiError = { error: string; code: string; activeRunId?: string | null };
