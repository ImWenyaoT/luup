...
test("live CLI rejects absent credentials before creating its SQLite database", async () => {
  ...
  const code = await main(["--ids", "1", "--db", dbPath], { nodeVersion: "v22.13.0" });
  assert.equal(code, 2);
...
test("formal Science-125 CLI requires an explicit confirmation before opening SQLite", async () => {
  ...
  const code = await main(["--ids", "1-125", "--db", dbPath], { nodeVersion: "v22.13.0" });
...
test("formal live batch requires the pinned Node runtime, while dry-run is portable", () => {
  assert.equal(validateBatchRuntime({ nodeVersion: "v22.13.0", dryRun: false }), null);
  assert.match(validateBatchRuntime({ nodeVersion: "v22.12.0", dryRun: false }) ?? "", /22\.12\.0/);
  assert.match(validateBatchRuntime({ nodeVersion: "v22.12.0", dryRun: false }) ?? "", /Node\.js 22.*22\.13\.0/);
  assert.match(validateBatchRuntime({ nodeVersion: "v23.3.0", dryRun: false }) ?? "", /23\.3\.0/);
  assert.match(validateBatchRuntime({ nodeVersion: "v23.3.0", dryRun: false }) ?? "", /Node\.js 23.*23\.4\.0/);
  assert.equal(validateBatchRuntime({ nodeVersion: "v23.4.0", dryRun: false }), null);
  assert.equal(validateBatchRuntime({ nodeVersion: "v24.0.0", dryRun: false }), null);
  assert.match(validateBatchRuntime({ nodeVersion: "v20.10.0", dryRun: false }) ?? "", /20\.10\.0/);
  assert.match(validateBatchRuntime({ nodeVersion: "v20.10.0", dryRun: false }) ?? "", /Node\.js 24 及以上/);
...
test("live batch rejects an unsupported Node runtime before opening SQLite", async () => {
  ...
  assert.match(output, /Node\.js 22.*22\.13\.0/);
  assert.match(output, /Node\.js 23.*23\.4\.0/);
  assert.equal(existsSync(dbPath), false);
});
...
test("formal preflight prints an admitted plan and never creates SQLite", async () => {
  ...
  const code = await main(
    ...
    { nodeVersion: "v22.13.0", modelCredential: true },
  );
...
test("formal preflight rejects missing credentials, dirty source, and existing DB before side effects", async () => {
  ...
  assert.equal(
    await main(
      ...
      { nodeVersion: "v22.13.0", modelCredential: false },
    ),
    2,
  );
  assert.match(output, /缺少 QWEN_API_KEY/);
...
test("preflight rejects manifest resume and dry-run instead of pretending to inspect durable facts", async () => {
  assert.equal(await main(["--manifest-id", "resume", "--preflight"], { nodeVersion: "v22.13.0" }), 2);
  assert.equal(await main(["--ids", "1-125", "--preflight", "--dry-run"], { nodeVersion: "v22.13.0" }), 2);
});
...
test("--dry-run with ids leaves no persistent manifest behind", async () => {
  ...
  assert.equal(await main(["--ids", "1", "--dry-run", "--db", path], { nodeVersion: "v22.13.0" }), 0);
