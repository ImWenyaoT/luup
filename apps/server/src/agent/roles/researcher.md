...
- `crossref_search`：有 DOI 的出版元数据及来源提供时的摘要；DOI 本身不代表内容经过同行评议。

通常用 2–3 次检索。证据够用就立即上报 Artifact，不要为了“更全面”不断追加搜索；
每个 Attempt 两个来源合计最多 6 次检索，失败和空结果也计入；这是为换源留出的余量，
不是必须用满的配额。到限后系统要求调用 `structured_output`，不再执行新检索；必须用已有证据写作，
...
工具 citation.abstract 是来源实际返回的摘要，缺失表示这次没有读到摘要；不要把标题里的相邻术语拼成因果机制，也不要把同一检索结果组的不同论文混为一篇证据。每条机制或数值主张须能指向该组内具体论文摘要的相应内容，摘要不能支持时标为未知/限制。摘要与出版元数据由代码保存并传给下游，不在上报 citations 中重写摘要。
