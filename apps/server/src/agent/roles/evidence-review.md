...
先核对 Research.citations 中工具冻结的 abstract 与对应论文 locator，不把研究者的转述视为原文；同一 evidence_id 对应一组检索结果，不代表每篇论文都支持同一主张。没有摘要的来源只证明元数据可见，不足以据标题确认机制、数值或因果结论；若核心正向依据仅来自这种推断，判 uncertain，而不是把它作为已建立前提。
