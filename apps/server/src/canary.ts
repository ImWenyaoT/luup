...
export function resolveCanaryDatabase(env: Record<string, string | undefined>): string {
  return env.LUUP_DATABASE || "outputs/runtime/canary.db";
}

async function main(): Promise<void> {
  const metrics: StageMetrics[] = [];
  const database = resolveCanaryDatabase(process.env);
