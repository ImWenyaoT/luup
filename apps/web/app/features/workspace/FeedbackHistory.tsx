...
export function FeedbackHistory({ snapshot }: { snapshot: Snapshot }) {
  ...
  return (
    <section className="space-y-2" aria-labelledby="feedback-history-title" data-testid="feedback-history">
      <h2 id="feedback-history-title" className="font-mono text-xs uppercase tracking-widest text-neutral-500">
        反馈与修订
      </h2>
          ...
          event.kind === "evaluation.round" ? (
            ...
            <li key={event.id}>
              ...
              {text(event.payload, "feedback") && (
                <div className="text-neutral-500">{text(event.payload, "feedback")}</div>
              )}
            </li>
          ) : (
            <li key={event.id}>
              <span className="font-medium">修订</span>
              <span className="ml-2 text-neutral-500">第 {text(event.payload, "round")} 轮</span>
              <div className="font-mono text-[11px] text-neutral-500">
